...
#Wait for 10 bauds after the end of a packet
UART_PACKET_END_TIMEOUT = 10 * int(CLOCK_FREQUENCY/BAUD_RATE)
...
class TB():
    ...
    def start(self):
        ...
        clock = Clock(self.dut.i_sys_clk_p, clock_period, "ps")
        cocotb.fork(clock.start())

    async def reset(self):
        self.dut.i_uart_rx.value = 1
        await ClockCycles(self.dut.i_sys_clk_p, 1)
    ...
    async def verify_receive(self, byte):
        ...
        while True:
            ...
            if self.dut.o_rx_en.value == 1:
                break 
            await RisingEdge(self.dut.i_sys_clk_p)
...
@cocotb.test()
async def test_uart_receive_comprehensive(device):
    ...
    await dut.transmit(0xaa)
    
    for i in range(20):
        await dut.wait_clock()

    await dut.transmit(0xff)
